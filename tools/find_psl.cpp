...
NVHM_NO_INLINE void fill_and_count_probe_seq_lengths() {
  using set_t = Set;
  ...
  // Find max probe sequence length.
  psl_t max_psl{};
  for (int_t i{}; i < num_keys;) {
    ...
    for (const key_t& k : keys) {
      set.for_each(k,
        [&](const read_pos_t&, const probe_seq_t& seq) {
          max_psl = std::max(max_psl, seq.psl());
        }
      );
    }
  }
  const int num_align{rendered_length(set.capacity())};
  // TODO: Check if we can improve incremental reinserts to offer a better distribution?
  //set.clear();
  set.clear(init_capacity);

  std::cout << "| " << std::setw(num_align) << "size";
  std::cout << " | " << std::setw(num_align) << "cap.";
  for (psl_t psl{}; psl <= max_psl; psl += set_t::kernel_size) {
    std::cout << " | " << std::setw(num_align) << psl;
  }
  std::cout << " |\n";

  std::cout << std::setfill('-') <<  "| " << std::setw(num_align) << "" << " | " << std::setw(num_align) << "";
  for (psl_t psl{}; psl <= max_psl; psl += set_t::kernel_size) {
  ...
  for (int_t i{}; i < num_keys;) {
    ...
    std::cout << "| " << std::setw(num_align) << set.size() << " | " << std::setw(num_align) << set.capacity();
    for (psl_t psl{}; psl <= max_psl; psl += set_t::kernel_size) {
      std::cout << " | " << std::setw(num_align);
      auto it{psl_counts.find(psl)};
      if (it != psl_counts.end()) {
        std::cout << it->second;
      } else {
        std::cout << "";
      }
...
void run_3(nvhm_type_t nvhm_type) {
  switch (nvhm_type) {
    case nvhm_type_t::map:
      return fill_and_count_probe_seq_lengths<set<Key, flags_t::none, Kernel, ProbeSeq>>();
    case nvhm_type_t::cache:
      return fill_and_count_probe_seq_lengths<cache_set<Key, flags_t::none, Kernel>>();
  }
